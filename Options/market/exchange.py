...
from Agents.SAC.blackscholes import BlaScho

class MarketExchange:
    
    def __init__(self, time, underlying_price=100, interval=5, option_depth=5):
        # stock ka price
        ...
        self.tickers = []
        self.market_books = {}
        self.master_ledger = TradeLedger()#ismei saare trades rakhe hai 
        self.atm = round(self.underlying_price / self.strike_interval) * self.strike_interval
        # These will be fixed for this simulation
        self.time_to_expiry = time # e.g., 25%
    ...
    def calculate_initial_premium(self, ticker_tuple):
        ...
        #black scholes
        option_type = 'call' if '_CE' in ticker_name else 'put'
        
        black = BlaScho(self.underlying_price, strike_price, self.time_to_expiry, option_type)
        ...
        # # 5% of stock ka underlying price
    ...
    def update_market(self, new_underlying_price):
        ...
        # Loop through all tickers to update their state.
        for ticker_tuple in self.tickers:
            ...
            if book:
                ...
                # purane orders jo out of context hai vvo hatta do
