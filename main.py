...
initial_exchange = MarketExchange(underlying_price=1000.0)  
...
for i in range(n_episodes):
    ...
    if days_to_expiry == option_cycle_days:
        ...
        # Update environments with the exchange 
    ...
    #dividing the agents into diffrent batches for psuedo asynchronus behaviour
    ...
    for step, price in enumerate(tqdm(daily_price_path, desc=f"Day {i+1} Progress")):
        ...
        all_insti_portfolios = {env.agent.agent_id: float(env.portfolio_value) for env in insti_envs}
        all_mm_portfolios = {agent.agent_id: float(agent.broker.portfolio_value) for agent in mm_agents}
        all_retail_portfolios = {env.agent.agent_id: float(calculate_retail_portfolio_value(env, price)) for env in retail_envs}

        # 3. Calling the single update function with all the new data
        update_data(
            price=float(price),
