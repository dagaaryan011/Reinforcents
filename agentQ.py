...
class AgentQ: 
    ...
    def train(self):
      ...
      for episode in tqdm(range(self.episodes)):
          ...
          action = self.getaction(self.epsilon)
          ...
          while step < 40 and self.env.playerpos != self.env.goal:
              ...
              # reward -= step
              #ab kya karajaye
              next_action = self.getaction(self.epsilon)
...
#-----------------------visulaize--------------
    def genimage(self,pos,episode):
        ...
        for i in range(1, size):
          ...
          cv2.imshow(f"GridRunner - Test {episode}", img)
          cv2.waitKey(20) 
...
if __name__ == "__main__":
    ...
    agent1 = AgentQ(env, episodes=10000,epsilon = 0.9, df = 0.6, lr= 0.5)  
    agent1.train()
