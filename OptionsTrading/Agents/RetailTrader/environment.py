...
class Envn:
    ...
    def get_trend_state(self):
        ...
        return [rsi, stoch, cci, cmf]
    
    def get_ticker_state(self, ticker):
        hb = self.highest_bids[ticker]
        la = self.lowest_asks[ticker]
        mid = self.mids[ticker]
        spr = self.spreads[ticker]
        pre = self.premiums[ticker]
        de = self.deltas[ticker]
        gam = self.gammas[ticker]
        the = self.thetas[ticker]
        veg = self.vegas[ticker]

        return [hb, la, mid, spr, pre, de, gam, the, veg]
    ...
    def get_RSI(self):#14 is std RSI period , change in price of stock pe RSI banate hai
    ...
    def get_CCI(self):
        ...
        if len(self.closes) <= 14:
            ...
            trend = self.closes[-14:]
        sma_today = np.mean(np.array(trend))
        constant = 0.015
        
        deviations = []
        for i in range(len(self.typical_prices)):
            deviations.append(abs(self.typical_prices[i] - sma_today))
        mean_deviation = sum(np.array(deviations))/ len(deviations)
        cci = (typical_price - sma_today) / (constant * mean_deviation)
        self.CCI = cci
    ...
    def ATR(self):
        ...
        curr_high = max(trend)
        curr_low = min(trend)
        x1 = curr_high - curr_low
        x2 = abs(curr_high - self.closes[-1])
        x3 = abs(curr_low - self.closes[-1])

        tr = max(x1, x2, x3)

        if len(self.true_ranges) < 14:
            self.true_ranges.append(tr)
        elif len(self.true_ranges) == 14:
            atr = np.mean(np.array(self.true_ranges))
            self.ATR = atr
            self.true_ranges.append(atr)
        else:
            atr = (self.atr*13 + tr) / 14
